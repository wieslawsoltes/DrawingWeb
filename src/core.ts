...
import { clone, createDocument, createPage, createShape, DrawingError, freeze, id, Signal, validateDocument } from './model.js';
import type { DiagramDocument, Matrix, Page, Shape } from './model.js';
import { parsePath, serializePath, transformSegments } from './path.js';
import { IDENTITY, inverse, localMatrix, multiply, transformPoint, transformedBounds, union } from './geometry.js';
...
export class DiagramEngine {
  ...
  transaction<T>(label: string, action: () => T, origin = 'user'): T {
    ...
    try {
      ...
      if (outer && this.value !== before) { validateDocument(this.value); this.value = freeze(this.value); this.reindex(); }
  ...
  update(shapeId: string, patch: Partial<Shape>, origin = 'user'): void {
    this.run('Update shape', () => {
      ...
      const safe = clone(patch); this.replaceAt(ref, { ...shape, ...safe, id: shapeId,
  ...
  remove(shapeIds: readonly string[], origin = 'user'): void {
    this.run('Delete shapes', () => {
      const ids = new Set(shapeIds.filter(key=>this.getShape(key))); if(!ids.size)return;
      const descend = (shape: Shape) => { ids.add(shape.id); shape.children?.forEach(descend); };
      for (const key of shapeIds) { const shape = this.getShape(key); if (shape) descend(shape); }
      for (const shape of this.allShapes()) if (ids.has(shape.source?.shapeId ?? '') || ids.has(shape.target?.shapeId ?? '')) ids.add(shape.id);
      const filter = (shapes: Shape[]): Shape[] => shapes.filter(shape => !ids.has(shape.id)).map(shape => shape.children ? { ...shape, children: filter(shape.children) } : shape);
      this.value = { ...this.value, pages: this.value.pages.map(page => ({ ...page, shapes: filter(page.shapes) })) }; this.indexDirty = true;
      for (const key of ids) this.pending.add(key);
    }, origin);
  }
  ...
  removePage(pageId: string): void { this.run('Delete page', () => { this.getPage(pageId); if (this.value.pages.length === 1) throw new DrawingError('LAST_PAGE', 'A document must retain one page.'); this.value = { ...this.value, pages: this.value.pages.filter(p => p.id !== pageId) }; this.indexDirty = true; }); }
  ...
  move(ids: readonly string[], dx: number, dy: number, origin = 'user'): void {
    this.run('Move shapes', () => { for (const key of this.roots(ids)) { const s = this.getShape(key)!, ref = this.getRef(key)!; const inv = inverse(ref.parentMatrix); if (!inv) continue; const a = transformPoint(inv, { x:0,y:0 }), b = transformPoint(inv, { x:dx,y:dy }); this.update(key, { x:s.x+b.x-a.x, y:s.y+b.y-a.y }); } }, origin);
  }
  ...
  group(ids: readonly string[] = this.selection): string | undefined {
    const keys = this.roots(ids); if (keys.length < 2) return undefined;
    ...
    this.run('Group shapes', () => {
      ...
      this.setSiblings(first.pageId, first.parentId, result); keys.forEach(key=>this.pending.add(key));this.pending.add(group.id);
    });
  ...
  ungroup(ids:readonly string[]=this.selection):void {
    ...
    this.run('Ungroup shapes',()=>{for(const key of this.roots(ids)){
      ...
      for(const shape of this.allShapes(ref.pageId)){const patch:Partial<Shape>={};if(shape.source?.shapeId===key)patch.source=center;if(shape.target?.shapeId===key)patch.target=center;if(Object.keys(patch).length)this.update(shape.id,patch);}
    }});this.select(children);
  }
  duplicate(ids:readonly string[]=this.selection,dx=24,dy=24):string[]{
    const result:string[]=[];this.run('Duplicate shapes',()=>{const roots=this.roots(ids);const map=new Map<string,string>();
      const reserve=(s:Shape)=>{map.set(s.id,id());s.children?.forEach(reserve);};roots.forEach(key=>reserve(this.getShape(key)!));
      const copy=(s:Shape):Shape=>({...clone(s),id:map.get(s.id)!,children:s.children?.map(copy),source:s.source?{...s.source,shapeId:map.get(s.source.shapeId??'')??s.source.shapeId}:undefined,target:s.target?{...s.target,shapeId:map.get(s.target.shapeId??'')??s.target.shapeId}:undefined});
  ...
  align(ids:readonly string[],mode:'left'|'right'|'top'|'bottom'|'center'|'middle'):void{
    this.run('Align shapes',()=>{const roots=this.roots(ids),rects=roots.map(key=>transformedBounds(this.getShape(key)!,this.getRef(key)!.matrix)),all=union(rects);
  ...
  instantiateMaster(masterId:string,pageId:string,x:number,y:number):string{const master=this.value.masters.find(m=>m.id===masterId);if(!master)throw new DrawingError('MASTER_NOT_FOUND',masterId);const ids=new Map<string,string>();const reserve=(s:Shape)=>{ids.set(s.id,id());s.children?.forEach(reserve);};reserve(master.shape);const rename=(s:Shape):Shape=>({...clone(s),id:ids.get(s.id)!,children:s.children?.map(rename),source:s.source?{...s.source,shapeId:ids.get(s.source.shapeId??'')??s.source.shapeId}:undefined,target:s.target?{...s.target,shapeId:ids.get(s.target.shapeId??'')??s.target.shapeId}:undefined});const shape=rename(master.shape);shape.masterId=masterId;shape.x=x;shape.y=y;return this.add(pageId,shape);}
  ...
  dispose():void{if(this.disposed)return;this.disposed=true;this.changed.clear();this.selectionChanged.clear();this.refs.clear();this.undoStack.length=this.redoStack.length=0;}
}
