import { deriveMasterInheritance, masterBinding } from './inheritance.js';
...
export class DiagramEngine {
  ...
  transaction<T>(label: string, action: () => T, origin = 'user'): T {
    ...
    try {
      ...
      if (outer && this.value !== before) {
        deriveMasterInheritance(this, before);
  ...
  instantiateMaster(masterId:string,pageId:string,x:number,y:number):string{const master=this.value.masters.find(m=>m.id===masterId);if(!master)throw new DrawingError('MASTER_NOT_FOUND',masterId);const ids=new Map<string,string>();const reserve=(s:Shape)=>{ids.set(s.id,id());s.children?.forEach(reserve);};reserve(master.shape);const rename=(s:Shape):Shape=>({...clone(s),id:ids.get(s.id)!,masterId,masterBinding:masterBinding(s),sheetId:undefined,container:s.container?{...s.container,memberIds:s.container.memberIds.map(key=>ids.get(key)??key)}:undefined,calloutTargetId:ids.get(s.calloutTargetId??'')??s.calloutTargetId,children:s.children?.map(rename),source:s.source?{...s.source,shapeId:ids.get(s.source.shapeId??'')??s.source.shapeId}:undefined,target:s.target?{...s.target,shapeId:ids.get(s.target.shapeId??'')??s.target.shapeId}:undefined});const shape=rename(master.shape);shape.masterId=masterId;shape.x=x;shape.y=y;return this.add(pageId,shape);}
