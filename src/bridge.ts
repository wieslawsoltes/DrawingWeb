...
import { exportSvg, readDrawing, writeVdx, writeVsdx } from './io.js';
...
interface Handle {
  ...
  source?:VisioPackage; table?:ObservableTable; binding?:DiagramBinding; dataSubscription?:Unsubscribe;
}
...
export function command(key:string,name:string,args:unknown[]=[]):unknown{
  const state=requireHandle(key),control=state.control,engine=control.engine;
...
export function dispose(key:string):void{const state=handles.get(key);if(!state||state.disposed)return;state.disposed=true;handles.delete(key);state.observer.disconnect();state.binding?.dispose();state.table?.dispose();state.dataSubscription?.();for(const unsubscribe of state.subscriptions)unsubscribe();state.control.dispose();state.control.engine.dispose();state.callback=undefined;}
